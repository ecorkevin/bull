...
Job.fromData = function(queue, jobId, data){
  var job, parsedData, parsedOpts;
  try{
    parsedData = JSON.parse(data.data);
    parsedOpts = JSON.parse(data.opts);
  }catch (e){
    job = new Job(queue, jobId, {}, {});
    return job.remove().then(function() {
      throw new Error('Parsing job data/opts failed');
    });
  }

  job = new Job(queue, parsedData, parsedOpts);
  job.jobId = jobId;
