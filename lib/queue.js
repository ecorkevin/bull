...
Queue.prototype.processJobs = function(){
  ...
  .catch(function (err) {
    if(!err.message.match(/no jobs to move/gi)){
  ...
  if(opts && opts.block === false){
    ...
    return this.bclient.brpoplpushAsync(this.toKey(src), this.toKey(dst),
      ...
      if(jobId){
        ...
        // reject here to avoid running code for processing a job
        return Promise.reject(new Error('No jobs to move from queue: ' + _this.name));
      }
    });
  }
};
